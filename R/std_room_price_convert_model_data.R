...

{
  dk_std_room_convert_model_into_data<-dbGetQuery(mycon,
                                                  ...
                                                  ((c.city='北京市' and b.ready_for_rent_date>='2018-02-01')  or b.ready_for_rent_date>='2017-11-01') 
                                                  and a.lighting='私有采光' and a.is_small_window='否' and a.shape='正常'  and a.has_storeroom!='有' and a.has_terrace!='有' and a.status not in ('不可租' ,'已下架')
...
#{
# dk_sql_data2<-dbGetQuery(mycon,"select distinct c.city,c.block,c.name as community,3 as bedroom_num_y, 1 as toilet_num_y,
#                                              '' as xianfang_days_y,
#                                              12 as area_y,
#                                              12 as living_area_y,
#                                              0  as has_toilet_y,
...
#   as.data.frame 
#}
#dk_std_room_convert_model_into_data<- inner_join(dk_sql_data1,dk_sql_data2, by =c("city", "block","community" ,"city_level"), copy = FALSE, suffix = c(".dk_sql_data1", ".dk_sql_data2")) %>% distinct 
...
write.csv(dk_data,paste("Results/",Sys.Date(),"dk小区间均价.csv"),row.names = FALSE)

#{
# ziroom_sql_data1<-dbGetQuery(mycon,"select distinct a.city,case when b.company is not null then b.block else a.business_district end as block,
...
#ziroom_std_room_convert_model_into_data<- inner_join(ziroom_sql_data1,ziroom_sql_data2, by =c("city", "community" ), copy = FALSE, suffix = c(".ziroom_sql_data1", ".ziroom_sql_data2")) %>% distinct 
...
ziroom_data <- ziroom_std_room_convert_model_into_data %>% 
  ...
  mutate(final_std_room_outprice_pre = ifelse(city == '北京', quantile(std_room_outprice_pre, 0.65), quantile(std_room_outprice_pre, 0.55)))

write.csv(ziroom_data,paste("Results/",Sys.Date(),"ziroom小区间均价.csv"),row.names = FALSE)
...
write.csv(xiaoqu_std_price,paste("Results/",Sys.Date(),"小区间均价.csv"),row.names = FALSE)
...
dk_room_price_data <- left_join (dk_room_data, xiaoqu_std_price %>% ungroup() %>% select(city, community, xiaoqu_std_price_final) %>% distinct(), by =c("city","community"), copy = FALSE, suffix = c(".dk_room_data", ".xiaoqu_std_price")) %>% distinct() %>% 
  left_join (xiaoqu_std_price %>% select(city, block.dk_data, block_std_price) %>% distinct,  by =c("city" = "city","block" = "block.dk_data"), copy = FALSE, suffix = c(".dk_room_data", ".xiaoqu_std_price")) %>% distinct() %>% 
  write.csv(paste("Results/", Sys.Date(), "定价中间数据.csv"))
...
room_forcast_colnams <- c('origin_bedroom_num', 'bedroom_num', 'toilet_num', 'total_area', 'dw_num', 'dly_num',
                          ...
                          'lighting', 'is_small_window', 'shape', 'city_level', 'dts','dk_std_room_outprice', 'dk_block_price',
...
dk_room_price_data <- mutate(dk_room_price_data, 
                             room_outprice_pre = xiaoqu_std_price_final - predict(room_outprice_pre_model_has_balcony_area, dk_room_price_data1)) %>% 
...
write.csv(dk_room_price_data, paste("Results/",Sys.Date(),"新房定价结果.csv"),row.names=F)
